...
int main(void)
{
    ...
    */
    ...
    Model shark2("Microsub.obj", "3D/textures/MicroSub_Albedo.png");
    loaded = shark2.loadMesh();
    shark2.setTexture2();
    shark2.fillVertexData();
    shark2.apply();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 2nd obj
        glBindVertexArray(shark2.getVAO());
        shark2.loadModel(x + 5.f, y, z, 0.1f, rot_x, rot_y, rot_z, theta);

        tex0Address = glGetUniformLocation(shaderProg, "tex0");
        glBindTexture(GL_TEXTURE_2D, shark2.getTexture());
        glUniform1i(tex0Address, 0);
        ...
        glUniformMatrix4fv(transformLoc,
            1,
            GL_FALSE,
            glm::value_ptr(shark2.getTransMatrix()));

        glDrawArrays(GL_TRIANGLES, 0, shark2.getVertexData().size() / 8);
